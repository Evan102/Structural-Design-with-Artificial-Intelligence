...
class DeepQAgent(Agent):
    ...
    def __init__(self,
        ...
        self._target_q_network.to(self._device).eval()
...
def train(agent: Agent,
    ...
    df_scores = pd.DataFrame(columns=['eposide','average_score','score'])
    df_scores.to_csv(os.path.join(folder, "history.csv"), index=False)
    ...
    for i in range(start_epsiode, number_episodes):
        ...
        pd.DataFrame(EposideScores).to_csv(os.path.join(folder, "history.csv"), mode='a', index=False, header=False)
